...
- [x] Asignación de tareas a miembros específicos.
- [x] Implementación de filtros por estado.
- [x] Arquitectura basada en el patrón **MVC**.
- [x] Chatbot con IA para creación automática de tareas.
...
└── frontend/
    ...
    ├── package.json
    └── src/
        ├── data/sources/remote/api/ApiDelivery.tsx
        ├── data/repositories/…  (Project, Task, Chatbot)
        ├── domain/entities/…    (Project, Task, ChatMessage)
        └── presentation/
            ├── theme/AppTheme.tsx
            ├── components/ (TopBar, BottomNav, CircleProgress, LinearProgress)
            ├── navigator/MainStackNavigator.tsx
            └── views/
                ├── welcome/ (Welcome, Login, Register)
                ├── role/RoleChoose.tsx         
                ├── dashboard/Dashboard.tsx     
                ├── tasks/ (Tasks, CreateTask)   
                ├── projects/ (CreateProject, JoinProject, ProjectDetail)
                ├── chatbot/Chatbot.tsx        
                └── profile/Profile.tsx
```
